...
// add by seacat for load whole ogg file

size_t __cdecl mread(void* p, size_t s1, size_t s2, whole_mem_file* m)
{
	int l = s2;
	if (m->curr_pos+s2 > m->mem_size) l = m->mem_size-m->curr_pos;
	memcpy((unsigned char*)p, m->data+m->curr_pos, l);
	m->curr_pos += l;
	return l;
}

int __cdecl mseek(whole_mem_file* m, long s, int n)
{
	int l;
	switch(n)
	{
	case SEEK_CUR:
		l = m->curr_pos + s;
		if (n>m->mem_size || n<0) return -1;
		m->curr_pos = l; 
		break;
	case SEEK_END:
		m->curr_pos = m->mem_size;
		break;
	case SEEK_SET:
		if (s>m->mem_size || s<0) return -1;
		m->curr_pos = s;
		break;
	}
	return 0;
...
long __cdecl mtell(whole_mem_file* m)
{
	return m->curr_pos;
}

int __cdecl mclose(whole_mem_file* m)
{
	return 0;
}
...
static int _mseek64_wrap(whole_mem_file *f,ogg_int64_t off,int whence){
	if(f==NULL)return(-1);
	return mseek(f,off,whence);
}
...
int ov_open_mem(whole_mem_file *f,OggVorbis_File *vf,char *initial,long ibytes){
	ov_callbacks callbacks = {
		(size_t (*)(void *, size_t, size_t, void *))  mread,
			(int (*)(void *, ogg_int64_t, int))              _mseek64_wrap,
			(int (*)(void *))                             mclose,
			(long (*)(void *))                            mtell
	};
...
int ov_test_mem(whole_mem_file *f,OggVorbis_File *vf,char *initial,long ibytes){
	ov_callbacks callbacks = {
		(size_t (*)(void *, size_t, size_t, void *))  mread,
			(int (*)(void *, ogg_int64_t, int))             _mseek64_wrap,
			(int (*)(void *))                             mclose,
...
long ov_read(OggVorbis_File *vf,void *buffer,int bytes_req,int *bitstream){
  ...
  while(1){
	...
	if(samples>0){
	  ...
	  return channels == 1 ? (samples<<1) : (samples<<2);//seacat
	}
	return samples;
      }
    }
