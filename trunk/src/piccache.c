...
int JY_LoadImzFile(const char *filename, int id)
{
	...
	if (id < 0 || id >= PIC_FILE_NUM){  // id≥¨≥ˆ∑∂Œß
		return 1;
	}

	if (pic_file[id].pcache){        // Õ∑≈µ±«∞Œƒº˛’º”√µƒø’º‰£¨≤¢«Â¿Ìcache
		int i;
		for (i = 0; i < pic_file[id].num; i++){   //—≠ª∑»´≤øÃ˘Õº£¨
			tmpcache = pic_file[id].pcache[i];
			if (tmpcache){       // ∏√Ã˘Õº”–ª∫¥Ê‘Ú…æ≥˝
				if (tmpcache->s != NULL)
					SDL_FreeSurface(tmpcache->s);       //…æ≥˝±Ì√Ê
				list_del(&tmpcache->list);              //…æ≥˝¡¥±Ì
				SafeFree(tmpcache);                  // Õ∑≈cacheƒ⁄¥Ê
				currentCacheNum--;
			}
		}
		SafeFree(pic_file[id].pcache);
	...
	if (g_PreLoadPicGrp == 1){   //grpŒƒº˛∂¡»Îƒ⁄¥Ê
		pic_file[id].grp = (unsigned char*)malloc(pic_file[id].filelength);
		if (pic_file[id].grp == NULL){
			JY_Error("JY_PicLoadFile: cannot malloc grp memory!\n");
			return 1;
...
int JY_LoadImzPic(int fileid, int picid, int x, int y, int flag, int value)
{
	...
	if (fileid < 0 || fileid >= PIC_FILE_NUM || picid < 0 || picid >= pic_file[fileid].num)    // ≤Œ ˝¥ÌŒÛ
		return 1;

	if (pic_file[fileid].pcache[picid] == NULL){   //µ±«∞Ã˘Õº√ª”–º”‘ÿ
		//…˙≥…cache ˝æ›
		newcache = (struct CacheNode *)malloc(sizeof(struct CacheNode));
		if (newcache == NULL){
			JY_Error("JY_LoadPic: cannot malloc newcache memory!\n");
		...
		if (currentCacheNum < g_MAXCacheNum){  //cache√ª¬˙
			list_add(&newcache->list, &cache_head);    //º”‘ÿµΩ±ÌÕ∑
			currentCacheNum = currentCacheNum + 1;
		}
		else{   //cache “—¬˙
			tmpcache = list_entry(cache_head.prev, struct CacheNode, list);  //◊Ó∫Û“ª∏ˆcache
			pic_file[tmpcache->fileid].pcache[tmpcache->id] = NULL;
			if (tmpcache->s != NULL)
				SDL_FreeSurface(tmpcache->s);       //…æ≥˝±Ì√Ê
			list_del(&tmpcache->list);
		...
		newcache = pic_file[fileid].pcache[picid];
		list_del(&newcache->list);    //∞—’‚∏ˆcache¥”¡¥±Ì’™≥ˆ
		list_add(&newcache->list, &cache_head);    //‘Ÿ≤Â»ÎµΩ±ÌÕ∑
	}

	if (newcache->s == NULL){   //Ã˘ÕºŒ™ø’£¨÷±Ω”ÕÀ≥ˆ
		return 1;
	}
	if (flag & 0x00000001){
		xnew = x;
